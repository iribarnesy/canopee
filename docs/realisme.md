...
feston d'un pavage de disques) → 92 % (une guilde entière entre sans une ligne
de mécanisme : les chauves-souris arboricoles héritent de la loge qu'un pic a
taillée, et l'individu y devient une **colonie**. Aucun point gagné, J10 était
déjà vert : ce lot était le contrôle d'une promesse que le module s'était faite,
et elle tient — deux fiches, rien d'autre. Ce qu'il a coûté est dans ce qu'on
déclare, et la noctule est repartie **sans table** parce que ce qu'elle mange
n'est pas dans ce moteur)**.*
...
| C14 | Les bases échangeables suivent un bilan conservatif | ✅ | `bases.test.ts` — la variation du pool vaut altération + dépôts + litière − lessivage − charge acide, à l'arrondi près. Comme pour N, P et K, et avec la même réserve : la conservation ne valide pas le **niveau**. **Et elle ne valide pas non plus le domaine, ce qui a coûté un défaut de cent ans** (#234) : le pool était débité sans plancher et passait sous zéro — un stock de bases échangeables négatif n'existe pas, quand le complexe est vide ce sont les protons et l'aluminium qui occupent les sites. Mesuré, lande sèche telle qu'elle est déclarée, une graine : première cellule à l'an **58** — exactement quand la lande se boise —, moyenne sous zéro vers l'an **105**, cellule la plus pauvre à **−13 eq/m²** à 150 ans, et le pool **profond** y passait aussi (−0,037 à l'an 120) parce que `lessivageBasesEq` rend un lessivage négatif sur un stock négatif et que le tick l'ajoute au sous-sol : il perdait des bases qui n'avaient jamais existé. **Personne ne pouvait le voir** : `phDepuisSaturation` borne la saturation dans `[0,1]`, donc le pH — la seule grandeur que le reste du moteur consulte — restait au plancher, parfaitement correct, pendant que le stock plongeait ; et ce bilan-ci **passait**, à 1e-13, parce qu'il est cohérent avec lui-même. Il comptait simplement un stock impossible. Le complexe ne cède désormais que ce qu'il porte (`acideTamponnable`), et les protons qu'aucune base n'a neutralisés ne sont pas escamotés : ils ont leur propre poste hors du budget du pool (`basesAcideNonTamponneEqHa`), parce qu'ils n'ont jamais touché le pool — borner sans les compter aurait remplacé une création de bases négatives par une **destruction d'acidité**. Banc apparié, ajoncs sur lande sèche, une seule chose changée : cellule la plus pauvre **−1,132 → 0,000**, moyenne 1,856 → 1,864, budget refermé à 1e-13 des deux côtés. Sur la lande entière à 150 ans : cellule la plus pauvre **−9,768 → 0,000**, moyenne **−0,157 → 0,911** (un sol désaturé cesse de creuser et se stabilise bas), sous-sol **3,383 → 3,820**, et sur la période 3,037 eq/m² de charge acide tamponnés contre **0,916 sans rien en face** — près d'un quart de ce que la litière a produit. **L'audit qui a suivi a passé tous les pools conservatifs au même crible** — eau, azote, carbone, phosphore, potassium, bases — site de débit par site de débit, puis en mesure sur les sept stations, boisées et nues, cent ans, chaque cellule chaque semaine : **aucun autre négatif**, et la raison est structurelle. Partout ailleurs ce qu'on retire est une proportion du stock lui-même ou un `Math.min` contre lui ; les bases étaient le seul endroit où une quantité **absolue**, calculée à partir d'autre chose, était retranchée d'un stock. L'invariant est désormais gardé pour tous les pools à la fois (`pools-positifs.test.ts`), sur des décors calibrés à l'envers : plancher retiré, ils tombent. Hors d'atteinte et dit : **ce que ces protons font à la matrice minérale**. Sous un certain taux de saturation c'est elle qui encaisse et elle relargue de l'aluminium, toxique pour les racines — la gamme tampon d'Ulrich que `PH_PLANCHER` nomme déjà sans la simuler. Le poste dit combien de protons y arrivent, pas ce qu'ils y font |
...
**Et cette dernière exclusion vient de tomber en partie** (#212). Elle n'était
pas un jugement sur la qualité des mesures méditerranéennes : elle disait qu'il
n'existait aucune station du moteur où les poser, donc que les y confronter
aurait comparé deux climats. Depuis qu'une suberaie des Maures existe, avec la
série réelle du Luc (Var), **une référence méditerranéenne est recevable — sur
cette station-là**. Ce qui ne rend pas les trois essences calées pour autant :
le blocage est levé, le calage reste à faire, et c'est un lot avec son témoin.
...
| Chêne-liège | 0,30 | 0,30 | Sánchez-González 2010 (Espagne, Tunisie) | **pas encore vérifié** : la station méditerranéenne existe depuis #212, la confrontation est le lot suivant — et elle devra trancher entre les 5,0 m à quarante ans d'une suberaie naturelle dense et les ~11 m d'une jeune plantation portugaise, ce qui est décider ce que le jeu modélise |
