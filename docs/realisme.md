...
| C. Nutriments et cycles | 17 | 1 | 0 | 18 |
...
**Score de réalisme : 130 pleins + 19 partiels sur 153 → 91 %** *(un partiel compte 1/2)*.
...
| C15 | La POMPE À BASES : un feuillu remonte les bases du sous-sol et les dépose en surface, appauvrissant la profondeur | 🟡 | `bases.ts` (`prelevementProfondEq`) ; `pompe-bases.test.ts` — le moteur tient désormais un pool de bases de SOUS-SOL, débité par les racines : le calcium qu'un arbre met dans sa litière, il l'a pris, et d'autant plus bas que son système racinaire descend. Deux traits de l'atlas suffisent (`litiere.calciumMgG`, `racines.profondeurMaxCm`), aucune espèce n'est nommée. Mesuré sur cinquante ans, limon riche : une hêtraie retire 8 100 eq/ha au fond (695 600 → 687 500, pH 7,00 → 6,95), une pinède 170 — le pin descend pourtant deux fois plus bas, et c'est bien la TENEUR qui décide, pas la profondeur. **🟡 et pas ✅, pour une raison chiffrée** : ce pool n'a que ce terme-là. L'altération, qui devrait l'alimenter, somme sur tout le profil et crédite encore la surface — 9 400 eq/ha sur cinquante ans, soit le MÊME ordre de grandeur que la pompe. `basesProfondEq` compte donc ce qu'un peuplement a pompé, il ne dit pas ce que le sous-sol vaut. Et le contraste de Foltran et al. (hêtre acidifiant le fond PLUS que l'épicéa) reste hors d'atteinte : l'atlas n'a pas d'épicéa |
