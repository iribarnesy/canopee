...
- **Temps de travail : budget annuel en UTH** (unité de travail humain, le terme agricole pour « équivalent temps plein » — on garde « ETP » pour l'évapotranspiration afin d'éviter la collision de sigles). 1 UTH ≈ 1 800 h/an ; le joueur seul = 1 UTH. Chaque action a un coût horaire réaliste (planter un arbre ~1 h, récolter 100 kg de pommes ~3 h *(à calibrer)*). Deux compteurs : les **heures de la semaine** et la **moyenne annuelle glissante en UTH** — pointer à 70 h une semaine de plantation est OK tant que l'année reste ≤ 1 UTH. Les 60 h/UTH hebdomadaires ne sont **pas un mur** : le moteur n'a jamais refusé de travail parce qu'il en restait trop peu dans la semaine, il facture ce qui dépasse. Chaque tranche de 60 h au-delà du plafond vaut une **embauche saisonnière** (`coutDuDepassement`, §10 du moteur : `depassementHoraire` donne les heures, la tranche coûte un `SEASONAL_EUR_WEEK`). Le joueur compose donc sa semaine librement et lit la facture ; c'est l'argent qui arbitre, pas un garde-fou. Pour dépasser le budget annuel il faut **embaucher** (saisonnier ponctuel ou salarié permanent, € ↔ UTH) : on démarre tout seul, on salarie quand la ferme le permet. Récolte non faite = perdue (ou tombée = litière). C'est ce qui rend la forêt-jardin mature payante : peu d'heures, du rendement (ch6, « le pari des vivaces »).
...
4. Temps de travail budgété **à l'année en UTH**, avec compteur hebdomadaire. Le seuil hebdomadaire (60 h/UTH) est un **seuil de facturation**, pas un refus : au-delà, chaque tranche entamée coûte une embauche saisonnière (§10).
