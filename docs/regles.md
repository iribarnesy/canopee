...
- **VENT : dérivé, pas saisi** (`vent.ts`, construit le 2026-09-11). Les séries du dépôt ne portent que quatre colonnes (T moyenne, min, max, pluie) : il n'y a pas de relevé de vent à lire, et le moteur n'a longtemps tenu que `Station.ventExposition` — un scalaire d'ABRI ∈ [0,1] dérivé des boisements voisins, ni direction ni variation. Le vent se calcule donc comme l'ETP, depuis ce que la semaine dit déjà et une propriété déclarée de la station :
  - **la station porte une rose à DEUX secteurs** — celui du régime perturbé et celui du régime anticyclonique sec, avec la force relative du second. Déclarée comme la latitude, parce que c'est un relevé et non une conséquence du sol. La rose par défaut est atlantique : ouest-sud-ouest en régime perturbé, est-nord-est et faible par beau temps, ce qui vaut pour la plus grande partie du territoire ;
  - **la pluie est le marqueur de régime**, et c'est le meilleur dont le moteur dispose déjà : en France la pluie vient massivement des perturbations d'ouest, donc une semaine arrosée est une semaine de flux zonal — secteur dominant, vent soutenu — et une semaine sèche une semaine anticyclonique. Le passage de l'un à l'autre est une interpolation et non un basculement, sinon la direction sauterait d'un secteur à l'autre une semaine sur deux ;
  - **deux exceptions régionales inversent la seconde règle, et ce sont les vents de feu** : le mistral (vallée du Rhône, nord à nord-nord-ouest) et la tramontane (Languedoc, nord-ouest) soufflent précisément par beau temps sec, et fort. D'où la rose à deux secteurs plutôt qu'un, et `ROSE_MISTRAL` / `ROSE_TRAMONTANE` fournies pour les stations réelles à venir ;
  - **le vent ne dépend PAS de la graine de la partie** : c'est de la météo, et la météo est une entrée de ce moteur, pas un tirage. Deux parties qui traversent la même semaine de la même série voient passer la même dépression. Sa dispersion résiduelle se tire sur une graine PROPRE, comme la direction de chute d'une chandelle, pour ne pas décaler le flux principal et rebattre les cartes de tous les autres mécanismes ;
  - **la force moyenne d'une année retombe sur `ventExposition`** — mesuré à 3 % près sur les trois stations qui portent une série. C'est la contrainte qui permet d'ajouter le vent à un moteur déjà calibré : `indiceRisqueFeu` prend maintenant la force de la semaine au lieu de l'exposition du lieu, donc le risque se concentre sur les semaines ventées au lieu d'être étalé sur toutes, sans que la climatologie du feu d'une station se déplace. La force peut DÉPASSER un : un mistral est exceptionnel, et c'est à l'usage de borner s'il en a besoin.
  - *(reste ouvert : une propagation ANISOTROPE dans le sens du vent — un vrai front s'allonge en ellipse au lieu de s'étaler en tache — et l'orientation des chablis par le vent, qui ne suivent aujourd'hui que la pente.)*
- **L'incendie rapporte la CHARGE de chaque cellule brûlée** (`IncendieResult.charges`), à côté du rang d'arrivée du front. Le rang dit où le feu est passé et quand ; la charge dit dans quoi il a brûlé, donc avec quelle violence. Elle décidait déjà de l'intensité, donc de qui meurt (`intensiteDuFeu`, sortie du tick où elle vivait en une ligne anonyme) — mais personne ne pouvait la relire.
