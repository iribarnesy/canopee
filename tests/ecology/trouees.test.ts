...
describe("une trouée dans un couvert fermé", () => {
  ...
  it("ce n'est pas « plus de semis », c'est un TRI : le pionnier n'entre que par la trouée", () => {
    ...
    //
    // **LE COMPARATEUR CROISÉ EST RETIRÉ À SON TOUR, ET POUR LA MÊME RAISON
    // QUE LE PRÉCÉDENT.** Il comparait la trouée à la zone témoin DE LA MÊME
    // PARTIE, ce qui suppose que cette zone-là soit fermée. Depuis
    // l'auto-éclaircie (#96), un peuplement dense se troue TOUT SEUL : la
    // « zone témoin » n'est plus un couvert fermé, c'est un morceau de forêt
    // qui s'éclaircit comme le reste. Mesuré sur les cinq graines, bouleaux
    // dans la trouée / dans la même zone sans trouée / dans la zone témoin de
    // la partie trouée :
    //
    //   10 / 6 / 11     13 / 2 / 6     14 / 0 / 2     9 / 3 / 4     9 / 0 / 2
    //
    // La première graine met la zone témoin DEVANT la trouée. Ce n'est pas que
    // la trouée n'y fait rien — elle y fait 10 contre 6 — c'est que le témoin y
    // est ouvert. Exactement la prémisse fausse qui avait coûté l'essai
    // précédent, et on ne la rattrape pas par un seuil plus bas.
    //
    // Ce qui reste, et qui suffit : le comparateur APPARIÉ, la même zone avec
    // et sans trouée, qui tient 5/5 avec de larges marges.
    for (const p of parties) {
