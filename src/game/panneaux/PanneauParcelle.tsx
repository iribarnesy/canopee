    ...
    <dl className="stats">
      ...
      {(snapshot.fluxes.boisSedimentPiegeKgM2 > 0 || snapshot.fluxes.boisRetenueMm > 0) && (
        <>
          ...
          <dd>
            ...
            <strong>{(snapshot.fluxes.boisSedimentPiegeKgM2 * 520_000).toFixed(0)} kg/ha/an</strong>{" "}
            de terre
            <span className="detail">
              {" "}
              · un tronc couché en travers de la pente met l'eau en flaque, le temps qu'elle rentre,
              et fait déposer derrière lui ce que le ruissellement emportait
            </span>
          </dd>
        </>
      )}
      {snapshot.fluxes.partInondee > 0 && (
        <>
          <dt>Crue</dt>
          <dd>
            la nappe affleure sur {(snapshot.fluxes.partInondee * 100).toFixed(0)} % de la parcelle
          </dd>
        </>
      )}
      <dt>Biodiversité</dt>
